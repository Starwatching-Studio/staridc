...
/**
 * 彩虹聚合登录处理
 * 文档：https://login.az0.cn
 */

// 聚合登录支持的登录方式
// icon_type: fa=Font Awesome图标, svg=内嵌SVG路径, custom=后台自定义(图片URL或文字)
$OAUTH_TYPES = [
...
/**
 * 判断是否需要背景色（custom类型且有图片URL时不需要背景色）
 */
...
// 仅当直接访问 oauth.php 时执行路由逻辑（被 include 时不执行）
...
if ($act === 'bind') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $mode = $_POST['bind_mode'] ?? 'register'; // register 或 login
