    ...
    if ($step === 2) {
        ...
        if (empty($dbName) || empty($dbUser) || empty($adminUser) || empty($adminPwd)) {
            ...
            try {
                ...
                // 应用标准数据库结构：补齐安装脚本基础建表未涵盖的表/字段/索引，
                // 保证全新安装即与修复脚本(db_check.php)的标准结构完全一致。
                require_once ROOT . 'rd/db_fix.php';
                $installSchemaLog = [];
                applyDatabaseSchema($pdo, $installSchemaLog);
                ...
                $stmt3 = $pdo->prepare("INSERT INTO admins(username,password,nickname,role,status) VALUES(?,?,?, 'super', 1)");
                $stmt3->execute([$adminUser, $hashedPwd, $adminUser]);

                $configContent = "<?php\n\$dbconfig=array(\n\t'host' => '".addslashes($dbHost)."',\n\t'port' => {$dbPort},\n\t'user' => '".addslashes($dbUser)."',\n\t'pwd' => '".addslashes($dbPwd)."',\n\t'dbname' => '".addslashes($dbName)."',\n);\n\n// ===================== 可选覆盖（取消注释并修改）=====================\n// define('SITE_URL', 'https://your-site.example.com');    // 主站地址（留空自动探测）\n// define('DEBUG', false);\n?>";
