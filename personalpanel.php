...
$validTabs = ['info','points','bill','hosts','tickets','orders','messages','security','referral','api'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'sign') {
        $lock = signLockAcquire($user['id']);
        $today = date('Y-m-d');
        $yesterday = date('Y-m-d', time() - 86400);
        $chk = $DB->prepare("SELECT last_sign_date, sign_streak FROM users WHERE id=?");
        $chk->execute([$user['id']]);
        $row = $chk->fetch();
        if ($row && $row['last_sign_date'] === $today) {
            $error = L('panel_sign_already');
        } elseif (riskIsBlocked('sign', 'account', (string)$user['id']) || riskIsBlocked('sign', 'ip', getClientIp())) {
            $error = '今日签到过于频繁，已被限制，请明日再试';
        } else {
            $streak = ($row && $row['last_sign_date'] === $yesterday) ? intval($row['sign_streak']) + 1 : 1;
            $points = calcSignPoints($streak);
            $DB->prepare("UPDATE users SET last_sign_date=?, sign_streak=? WHERE id=?")->execute([$today, $streak, $user['id']]);
            addPoints($user['id'], $points, 'sign', '', '每日签到（连续' . $streak . '天）');
            riskRecord('sign', 'account', (string)$user['id']);
        ...
        if (!$vhost) {
            ...
            if ($user['points'] < $renewPrice) {
                ...
                if ($mnbtResult['success']) {
                    addPoints($user['id'], -$renewPrice, 'renew', 'host:' . $vhostId, '续费主机【' . $vhost['model_name'] . '】' . $months . '个月');
                    ...
                    try {
                        ...
                        $DB->prepare("INSERT INTO orders(order_no,user_id,type,amount,points,status,remark,params,paid_at) VALUES(?,?, 'host_renew',0,?,1,?,NOW())")
                            ->execute([$orderNo, $user['id'], $renewPrice, $remark, json_encode(['host_id' => $vhostId, 'duration_type' => $durationType])]);
                    } catch (Exception $e) {}
    ...
    if ($action === 'create_ticket') {
            ...
            if (empty($subject) || empty($content)) {
                ...
                try {
                    ...
                    $stmt2 = $DB->prepare("INSERT INTO ticket_replies(ticket_id,user_id,content,attachment) VALUES(?,?,?,?)");
                    $stmt2->execute([$ticketId, $user['id'], $content, $attachment]);
    ...
    if ($action === 'read_message') {
        ...
        if ($msgId > 0) {
            $DB->prepare("UPDATE messages SET is_read=1 WHERE id=? AND user_id=?")->execute([$msgId, $user['id']]);
        }
        $currentTab = 'messages';
    }

    if ($action === 'read_all_messages') {
        $DB->prepare("UPDATE messages SET is_read=1 WHERE user_id=?")->execute([$user['id']]);
        $currentTab = 'messages';
    }

    if ($action === 'enable_2fa') {
        if (!empty($user['two_factor_enabled'])) {
            $error = L('panel_2fa_already');
        } else {
    ...
    if ($action === 'confirm_2fa') {
        ...
        if (empty($secret)) {
            $error = L('panel_2fa_generate_first');
        } elseif (totpVerify($secret, $code)) {
            $DB->prepare("UPDATE users SET two_factor_secret=?, two_factor_enabled=1 WHERE id=?")->execute([$secret, $user['id']]);
            unset($_SESSION['totp_secret']);
            $stmt = $DB->prepare("SELECT * FROM users WHERE id=?");
            $stmt->execute([$user['id']]);
            $user = $stmt->fetch();
            $success = L('panel_2fa_enabled_ok');
        } else {
            $error = L('panel_2fa_wrong');
        }
        $currentTab = 'security';
    }

    if ($action === 'disable_2fa') {
        $DB->prepare("UPDATE users SET two_factor_secret=NULL, two_factor_enabled=0 WHERE id=?")->execute([$user['id']]);
        unset($_SESSION['totp_secret']);
        $stmt = $DB->prepare("SELECT * FROM users WHERE id=?");
        $stmt->execute([$user['id']]);
        $user = $stmt->fetch();
        $success = L('panel_2fa_disabled_ok');
        $currentTab = 'security';
    }
}
...
$orderList = [];
try {
    $oStmt = $DB->prepare("SELECT * FROM orders WHERE user_id=? ORDER BY id DESC LIMIT 100");
...
$messageList = [];
try {
    $um = $DB->prepare("SELECT COUNT(*) as c FROM messages WHERE user_id=? AND is_read=0");
    $um->execute([$user['id']]);
    $unreadMsgCount = intval($um->fetch()['c']);
    $mStmt = $DB->prepare("SELECT * FROM messages WHERE user_id=? ORDER BY id DESC LIMIT 100");
    $mStmt->execute([$user['id']]);
    $messageList = $mStmt->fetchAll();
} catch (Exception $e) {
    $messageList = [];
}
...
$billRows = [];
try {
    $bStmt = $DB->prepare("SELECT * FROM points_log WHERE user_id=? ORDER BY id DESC LIMIT 100");
    $bStmt->execute([$user['id']]);
    $billRows = $bStmt->fetchAll();
} catch (Exception $e) { $billRows = []; }
...
<?php
// 新用户专享弹窗：注册成功后弹一次（受广告总开关 + 新用户弹窗开关控制；与签到弹窗独立）
$showNewUserPopup = false;
if (!empty($_SESSION['new_user_popup'])) {
    $adCfgNU = get_ad_config();
    if (!empty($adCfgNU['enabled']) && !empty($adCfgNU['newuser_ad']['enabled'])) {
        $nu = $adCfgNU['newuser_ad'];
        $uidNU = intval($_SESSION['user_id']);
        $already = false;
        try {
            $rsNU = $DB->prepare("SELECT 1 FROM user_ad_popup_log WHERE user_id=? AND popup_type='newuser' LIMIT 1");
            $rsNU->execute([$uidNU]);
            $already = (bool)$rsNU->fetch();
        } catch (Exception $e) {}
        if (!$already) {
            $showNewUserPopup = true;
            try {
                $DB->prepare("INSERT IGNORE INTO user_ad_popup_log(user_id,popup_type,created_at) VALUES(?, 'newuser', NOW())")->execute([$uidNU]);
            } catch (Exception $e) {}
        }
    }
    unset($_SESSION['new_user_popup']);
}
if ($showNewUserPopup):
    $nuImg = trim($nu['image'] ?? '');
    $nuLink = trim($nu['link'] ?? '');
    $baseNU = rtrim(siteUrl(), '/');
    $nuImgSrc = ($nuImg !== '' && strpos($nuImg, 'http') === 0) ? $nuImg : ($nuImg !== '' ? $baseNU . '/' . ltrim($nuImg, '/') : '');
    if ($nuLink === '') {
        $nuLink = '#'; // 未配置链接则不跳转
    }
?>
<div id="newUserAdOverlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9998" onclick="if(event.target===this)closeNewUserAd()"></div>
...
<div id="newUserAdModal" style="display:none;position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:9999;width:min(90vw,420px);background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.35)">
    <span class="ad-label-tag" style="position:absolute;top:8px;left:8px;z-index:2">广告</span>
    <?php if ($nuLink !== '#'): ?>
    <a href="<?php echo h($nuLink); ?>" target="_blank" rel="noopener" style="display:block">
    <?php endif; ?>
        <?php if ($nuImgSrc !== ''): ?>
        <img src="<?php echo h($nuImgSrc); ?>" alt="新用户专享" style="width:100%;display:block;max-height:60vh;object-fit:cover">
        <?php else: ?>
        <div style="padding:40px 24px;text-align:center;background:linear-gradient(135deg,#eef2ff,#faf5ff)">
            <div style="font-size:2.4rem;margin-bottom:12px">🎁</div>
            <div style="font-size:1.2rem;font-weight:700;color:#4338ca;margin-bottom:8px">新用户专享福利</div>
            <div style="color:#6b7280">欢迎加入！祝您使用愉快</div>
        </div>
        <?php endif; ?>
    <?php if ($nuLink !== '#'): ?>
    </a>
    ...
    <div style="display:flex;justify-content:flex-end;padding:12px 16px;border-top:1px solid #eee">
        <button onclick="closeNewUserAd()" style="padding:8px 18px;border:none;border-radius:8px;background:#6366f1;color:#fff;cursor:pointer;font-weight:600">我知道了</button>
    </div>
</div>
<script>
function closeNewUserAd(){var o=document.getElementById('newUserAdOverlay');var m=document.getElementById('newUserAdModal');if(o)o.style.display='none';if(m)m.style.display='none';}
var _o=document.getElementById('newUserAdOverlay');var _m=document.getElementById('newUserAdModal');if(_o)_o.style.display='block';if(_m)_m.style.display='block';
</script>
<?php endif; ?>

<div class="panel-grid">
    <div class="panel-sidebar">
        <?php include_once ROOT . 'templates/sidebar.php'; ?>
    </div>
    ...
    <div class="panel-main">
        ...
        <?php if ($checkinSuccess): ?><?php renderCheckinAdModal(); ?><?php endif; ?>
        ...
        <div id="panel-info" class="panel-section">
            ...
            <div class="section-card" style="margin-top:16px">
                <h3><i class="fas fa-server"></i> 我的实例</h3>
                <?php if (empty($vhostList)): ?>
                <p style="color:#888;padding:12px 0">暂无主机，<a href="cart.php" style="color:#6c5ce7;text-decoration:none">去选购</a></p>
                <?php else: ?>
                <div style="display:flex;flex-direction:column;gap:12px;margin:12px 0">
                    <?php foreach (array_slice($vhostList, 0, 5) as $vh): ?>
                    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;background:#f8f9fa;border-radius:10px">
                        <div>
                            <div style="font-weight:600"><?php echo h($vh['model_name']); ?></div>
                            <div style="font-size:.85rem;color:#888"><?php echo h($vh['account']); ?> · 到期 <?php echo $vh['expire_time'] ? date('Y-m-d', strtotime($vh['expire_time'])) : '永久'; ?></div>
                        </div>
                        <a href="?tab=hosts" style="padding:6px 14px;border-radius:8px;border:1px solid #ddd;background:#fff;color:#6c5ce7;text-decoration:none;font-size:.85rem">管理</a>
                    </div>
                    <?php endforeach; ?>
                    <?php if (count($vhostList) > 5): ?>
                    <a href="?tab=hosts" style="color:#6c5ce7;text-decoration:none;font-size:.85rem">查看全部 <?php echo count($vhostList); ?> 台主机 →</a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
        ...
        <div id="panel-bill" class="panel-section">
            <div class="section-card">
                <div class="section-header">
                    <h3>积分账单</h3>
                    <span style="font-weight:600">💰 <?php echo $user['points']; ?> <?php echo L('points'); ?></span>
                </div>
                <div class="table-wrapper">
                <table>
                    <thead><tr><th>类型</th><th>增减</th><th>变动后余额</th><th>说明</th><th>时间</th></tr></thead>
                    <tbody>
                    <?php if(empty($billRows)): ?><tr><td colspan="5" class="empty-state">暂无积分记录</td></tr>
                    <?php else: foreach($billRows as $b):
                        $bt = ['sign'=>'签到','recharge'=>'充值','referral'=>'邀请','exchange'=>'兑换主机','renew'=>'续费','expire'=>'过期清理','adjust'=>'调整','other'=>'其他'][$b['type']] ?? $b['type'];
                    ?>
                    <tr>
                        <td><span class="badge badge-info"><?php echo h($bt); ?></span></td>
                        <td style="color:<?php echo $b['delta']>=0?'#059669':'#dc2626'; ?>;font-weight:600"><?php echo ($b['delta']>=0?'+':'').$b['delta']; ?></td>
                        <td><?php echo intval($b['balance']); ?></td>
                        <td class="muted"><?php echo h($b['remark'] ?? ''); ?></td>
                        <td class="muted"><?php echo $b['created_at']; ?></td>
                    </tr>
                    <?php endforeach; endif; ?>
                    </tbody>
                </table>
                </div>
                <p class="form-hint">仅展示最近 100 条积分变动，数据永久保存，可核对每一笔来源。</p>
            </div>
        </div>
            ...
            <div class="section-card">
                ...
                <div class="vhost-list">
                    ...
                    <div class="vhost-card">
                        ...
                        <?php
                        $panelUrl = trim(conf('host_panel_url', ''));
                        if (!empty($panelUrl)) {
                            $panelParam = trim(conf('host_panel_param', ''));
                            $panelHref = rtrim($panelUrl, '/') . '/';
                            if ($panelParam !== '') $panelHref .= '?' . rawurlencode($panelParam) . '=' . rawurlencode($v['account']);
                            echo '<a href="' . h($panelHref) . '" target="_blank" class="btn-primary btn-sm" style="background:linear-gradient(135deg,#0ea5e9,#0284c7);text-decoration:none;display:inline-block;text-align:center">' . L('panel_enter_panel') . '</a>';
                        }
                        ?>
            ...
            <div class="section-card">
                ...
                <div id="ticket-form-new" style="display:none;margin-bottom:20px;padding:20px;border-radius:12px;background:var(--bg-card,#f8f9fa)">
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label><?php echo L('panel_tickets_attachment'); ?></label>
                    ...
                    <div class="hp-ticket-item" onclick="toggleTicketDetail(<?php echo $tk['id']; ?>)">
                        ...
                        <div id="ticket-detail-<?php echo $tk['id']; ?>" class="hp-ticket-detail" style="display:none">
                            ...
                            <div class="hp-reply-item <?php echo $isAdminReply ? 'hp-reply-admin' : 'hp-reply-user'; ?>">
                                ...
                                <div class="hp-reply-attachment">
                                    <?php if (preg_match('/\.(png|jpe?g|gif|webp)$/i', $r['attachment'])): ?>
                                    <a href="<?php echo h($r['attachment']); ?>" target="_blank"><img src="<?php echo h($r['attachment']); ?>" alt="附件" style="max-width:160px;max-height:160px;border-radius:8px;border:1px solid #eee"></a>
                                    <?php else: ?>
                                    <a href="<?php echo h($r['attachment']); ?>" target="_blank" style="display:inline-flex;align-items:center;gap:6px;color:#2563eb;text-decoration:none"><i class="fas fa-file-alt"></i> <?php echo L('panel_tickets_attachment'); ?></a>
                                    <?php endif; ?>
                                </div>
                                <?php endif; ?>
                            </div>
        ...
        <div id="panel-orders" class="panel-section">
            <div class="section-card">
                <div class="section-header">
                    <h3><?php echo L('panel_orders_title'); ?></h3>
                </div>
                <?php if (empty($orderList)): ?>
                <div class="empty-state"><i class="fas fa-receipt" style="font-size:3rem;opacity:0.3;margin-bottom:16px;display:block"></i><p><?php echo L('panel_orders_empty'); ?></p></div>
                <?php else: ?>
                <div class="orders-table-wrap">
                    <table class="orders-table">
                        <thead>
                            <tr>
                                <th><?php echo L('panel_orders_no'); ?></th>
                                <th><?php echo L('panel_orders_type'); ?></th>
                                <th><?php echo L('panel_orders_remark'); ?></th>
                                <th><?php echo L('panel_orders_points'); ?></th>
                                <th><?php echo L('panel_orders_time'); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($orderList as $od): ?>
                            <tr>
                                <td class="mono"><?php echo h($od['order_no']); ?></td>
                                <td>
                                    <?php
                                    $otypeMap = [
                                        'points' => L('panel_orders_recharge'),
                                        'host_buy' => L('panel_orders_buy'),
                                        'host_renew' => L('panel_orders_renew'),
                                    ];
                                    echo h($otypeMap[$od['type']] ?? $od['type']);
                                    ?>
                                </td>
                                <td><?php echo h($od['remark'] ?: ($od['type'] === 'points' ? L('panel_orders_recharge') : '')); ?></td>
                                <td class="points-cell">-<?php echo intval($od['points']); ?></td>
                                <td class="muted"><?php echo h($od['paid_at'] ?: $od['created_at']); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <div id="panel-messages" class="panel-section">
            <div class="section-card">
                <div class="section-header">
                    <h3><?php echo L('panel_messages_title'); ?></h3>
                    <?php if ($unreadMsgCount > 0): ?>
                    <form method="post" style="margin:0">
                        <?php echo csrfField(); ?>
                        <input type="hidden" name="action" value="read_all_messages">
                        <button type="submit" class="btn-primary btn-sm"><?php echo L('panel_messages_read_all'); ?></button>
                    </form>
                    <?php endif; ?>
                </div>
                <?php if (empty($messageList)): ?>
                <div class="empty-state"><i class="fas fa-envelope-open-text" style="font-size:3rem;opacity:0.3;margin-bottom:16px;display:block"></i><p><?php echo L('panel_messages_empty'); ?></p></div>
                <?php else: ?>
                <div class="msg-list">
                    <?php foreach ($messageList as $msg): ?>
                    <div class="msg-item<?php echo $msg['is_read'] ? '' : ' unread'; ?>">
                        <div class="msg-head">
                            <span class="msg-title"><?php echo h($msg['title']); ?></span>
                            <span class="msg-time"><?php echo h($msg['created_at']); ?></span>
                        </div>
                        <div class="msg-body"><?php echo nl2br(h($msg['content'])); ?></div>
                        <?php if (!$msg['is_read']): ?>
                        <form method="post" class="msg-read-form">
                            <?php echo csrfField(); ?>
                            <input type="hidden" name="action" value="read_message">
                            <input type="hidden" name="msg_id" value="<?php echo $msg['id']; ?>">
                            <button type="submit" class="btn-sm" style="background:var(--gray-200,#e5e7eb);color:#666;border:none;padding:4px 12px;border-radius:8px;cursor:pointer"><?php echo L('panel_messages_mark_read'); ?></button>
                        </form>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
        ...
        <div id="panel-security" class="panel-section">
            <div class="section-card">
                <div class="section-header"><h3><?php echo L('panel_security'); ?></h3></div>
                <?php if (!empty($user['two_factor_enabled'])): ?>
                <div class="msg-item" style="border-color:#10b981;background:#f0fdf4">
                    <div class="msg-head"><span class="msg-title"><?php echo L('panel_2fa_enabled'); ?></span></div>
                    <div class="msg-body"><?php echo L('panel_2fa_enabled_desc'); ?></div>
                </div>
                <form method="post" style="margin-top:16px">
                    <?php echo csrfField(); ?>
                    <input type="hidden" name="action" value="disable_2fa">
                    <button type="submit" class="btn-primary btn-sm" style="background:#ef4444" onclick="return confirm('<?php echo addslashes(L('panel_2fa_disable_confirm')); ?>')"><?php echo L('panel_2fa_disable'); ?></button>
                </form>
                <?php elseif (!empty($_SESSION['totp_secret'])): ?>
                <?php $secSecret = $_SESSION['totp_secret']; $secUri = totpUri($secSecret, $user['email'], conf('site_name','StarIDC')); ?>
                <p class="muted" style="margin-bottom:12px"><?php echo L('panel_2fa_scan_tip'); ?></p>
                <div class="form-group">
                    <label><?php echo L('panel_2fa_secret'); ?></label>
                    <div class="copyable" style="display:inline-block;padding:8px 12px;background:#f1f5f9;border-radius:8px;font-family:monospace;letter-spacing:1px"><?php echo h($secSecret); ?></div>
                </div>
                <form method="post">
                    <?php echo csrfField(); ?>
                    <input type="hidden" name="action" value="confirm_2fa">
                    <div class="form-group">
                        <label><?php echo L('panel_2fa_code'); ?></label>
                        <input type="text" name="code" inputmode="numeric" maxlength="6" required placeholder="6 位动态码" style="max-width:200px">
                    </div>
                    <button type="submit" class="btn-primary btn-sm"><?php echo L('panel_2fa_confirm'); ?></button>
                    <a href="?tab=security" class="btn-sm" style="background:var(--gray-200,#e5e7eb);color:#666;border:none;padding:6px 16px;border-radius:8px;cursor:pointer;margin-left:8px"><?php echo L('cancel'); ?></a>
                </form>
                <?php else: ?>
                <div class="msg-item">
                    <div class="msg-head"><span class="msg-title"><?php echo L('panel_2fa_not_enabled'); ?></span></div>
                    <div class="msg-body"><?php echo L('panel_2fa_not_enabled_desc'); ?></div>
                </div>
                <form method="post" style="margin-top:16px">
                    <?php echo csrfField(); ?>
                    <input type="hidden" name="action" value="enable_2fa">
                    <button type="submit" class="btn-primary btn-sm"><?php echo L('panel_2fa_enable'); ?></button>
                </form>
                <?php endif; ?>
            </div>
        </div>
...
.orders-table th,.orders-table td{padding:12px 14px;text-align:left;border-bottom:1px solid var(--gray-200,#eef0f3)}
.orders-table th{background:var(--bg-card,#f8f9fa);color:var(--gray-500,#64748b);font-weight:600;font-size:.82rem}
.orders-table .mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:.82rem}
.orders-table .points-cell{color:#e74c3c;font-weight:700}
.orders-table .muted{color:var(--gray-500,#64748b);font-size:.82rem}
.nav-badge{display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;padding:0 5px;margin-left:6px;border-radius:9px;background:#ef4444;color:#fff;font-size:.7rem;font-weight:700;line-height:18px}
.msg-list{display:flex;flex-direction:column;gap:12px}
.msg-item{border:1px solid var(--gray-200,#eef0f3);border-radius:12px;padding:14px 16px;background:#fff}
.msg-item.unread{border-color:#fcd34d;background:#fffdf5}
.msg-head{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px}
.msg-title{font-weight:600;color:#1e293b}
.msg-time{color:var(--gray-500,#64748b);font-size:.78rem;white-space:nowrap}
.msg-body{color:var(--gray-700,#475569);font-size:.9rem;line-height:1.6;white-space:pre-wrap}
.msg-read-form{margin-top:10px}
.user-ad-block{display:flex;flex-direction:column;gap:8px;margin-bottom:16px;padding:12px;border-radius:12px;background:linear-gradient(135deg,rgba(245,158,11,0.12),rgba(245,158,11,0.04));border:1px solid rgba(245,158,11,0.25);position:relative}
.ad-label-tag{display:inline-block;font-size:.68rem;color:#fff;background:rgba(0,0,0,.45);padding:1px 6px;border-radius:3px;line-height:1.5;letter-spacing:.5px}
.user-ad-block a{display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:10px;text-decoration:none;font-weight:600;font-size:.92rem;transition:all .2s}
.ad-custom{background:#fff;color:#d97706;border:1px solid rgba(245,158,11,0.3)}
.ad-custom:hover{background:#fffbeb;color:#b45309}
