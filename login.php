...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'register') {
        ...
        if (strlen($password) < 6) {
            ...
            $error = L('login_error_captcha');
        } elseif (riskIsBlocked('register', 'ip', getClientIp())) {
            $error = '注册请求过于频繁，请稍后再试';
        } elseif (riskIsBlocked('register', 'account', $email)) {
            $error = '该邮箱注册过于频繁，请稍后再试';
        } else {
            if ($mailEnabled) {
            ...
            if (empty($error)) {
                ...
                if (empty($error)) {
                    ...
                    if ($referrerId && $referralReward > 0) {
                        ...
                        addPoints($referrerId, $referralReward, 'referral', 'user:' . $newUserId, '邀请好友注册奖励');
                    ...
                    $_SESSION['new_user_popup'] = 1; // 标记：注册成功后弹一次新用户专享广告
                    riskRecord('register', 'ip', getClientIp());
                    riskRecord('register', 'account', $email);
                    Captcha::clearImage();
    ...
    if ($action === 'verify_2fa') {
        ...
        $uid = intval($_SESSION['pending_2fa_user'] ?? 0);
        if ($uid <= 0) {
            $error = L('login_2fa_expired');
            unset($_SESSION['pending_2fa_user'], $_SESSION['pending_2fa_remember']);
        } else {
            $stmt = $DB->prepare("SELECT * FROM users WHERE id=?");
            $stmt->execute([$uid]);
            $u = $stmt->fetch();
            if (!$u || empty($u['two_factor_enabled']) || !totpVerify($u['two_factor_secret'], $code)) {
                $error = L('login_2fa_wrong');
            } else {
                unset($_SESSION['pending_2fa_user'], $_SESSION['pending_2fa_remember']);
                $_SESSION['user_id'] = $u['id'];
                if (!empty($_SESSION['pending_2fa_remember'])) setRememberToken($u['id']);
                redirect('personalpanel.php');
            }
        }
    }
...
<div class="auth-container">
    <div class="auth-card">
        ...
        <?php if (!empty($_SESSION['pending_2fa_user'])): ?>
        <form method="post" class="auth-form">
            <?php echo csrfField(); ?>
            <input type="hidden" name="action" value="verify_2fa">
            <div class="form-group">
                <label><?php echo L('login_2fa_code'); ?></label>
                <input type="text" name="code" inputmode="numeric" maxlength="6" required placeholder="<?php echo L('login_2fa_code_placeholder'); ?>" autofocus>
            </div>
            <button type="submit" class="btn-primary" style="width:100%"><?php echo L('login_2fa_confirm'); ?></button>
        </form>
        <p class="auth-switch"><a href="?mode=login"><?php echo L('login_2fa_back'); ?></a></p>
