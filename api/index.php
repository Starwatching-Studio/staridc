...
/**
 * StarIDC API
 * 认证方式：Header X-API-Key 或 Query 参数 api_key
 */
define('IN_SYS', true);
define('ROOT', __DIR__ . '/../');
...
switch ($action) {
    ...
    case 'sign':
        ...
        $DB->prepare("UPDATE users SET points = points + ?, last_sign_date = ? WHERE id = ?")->execute([$points, $today, $userId]);
        jsonExit(200, 'Sign-in successful', [
            'points_earned' => $points,
            'total_points' => (int)$user['points'] + $points,
        ]);
        break;
    ...
    case 'host_buy':
        ...
        $DB->prepare("UPDATE users SET points = points - ? WHERE id = ?")->execute([$itemPrice, $userId]);
    ...
    case 'host_renew':
        ...
        $DB->prepare("UPDATE users SET points = points - ? WHERE id = ?")->execute([$renewPrice, $userId]);
